...
import json
from distributors.models import Distributor, DistributorStock, StockTransfer, StockTransferItem, DistributorStockTransferItem, DistributorStockTransfer,DirectStockTransfer,DirectStockTransferItem
from django.contrib.auth.decorators import login_required
from main.decorators import check_mode, shop_required, check_account_balance,permissions_required,ajax_required
from distributors.forms import DistributorForm, StockTransferItemForm, StockTransferForm, DistributorStockTransferItemForm, DistributorStockTransferForm,DirectStockTransferItemForm,DirectStockTransferForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            response_data = {
                ...
                "message" : "Sales Man Successfully Updated.",
                "redirect" : "true",
        ...
        context = {
            "form" : form,
            "title" : "Edit Sales Man : " + instance.name,
            "instance" : instance,
...
def delete(request,pk):
    ...
    response_data = {
        ...
        "message" : "Sales Man Successfully Deleted.",
        "redirect" : "true",
...
def delete_selected_distributors(request):
    ...
    if pks:
        ...
        response_data = {
            "status" : "true",
            "title" : "Successfully Deleted",
            "message" : "Selected Sales Man(s) Successfully Deleted.",
            "redirect" : "true",
            "redirect_url" : reverse('distributors:distributors')
        }
    else:
...
def distributor_stocks(request):
    ...
    if query:
        ...
        title = "Sales man stocks - %s" %query
...
def distributor_stock(request,pk):
    ...
    context = {
        "instance" : instance,
        "title" : "Sales Man Stock: " + instance.product.name,
        "single_page" : True,
...
@ajax_required
@login_required
def delete_distributor_stock(request,pk):
    current_shop = get_current_shop(request)
    instance = get_object_or_404(DistributorStock.objects.filter(pk=pk,is_deleted=False,shop=current_shop))
    ...
    response_data = {
        ...
        "redirect" : "true",
        "redirect_url" : reverse('distributors:distributor_stocks')
    }
    return HttpResponse(json.dumps(response_data), content_type='application/javascript')
...
def create_user(request,pk):
    ...
    if request.method == "POST":
        ...
        if instance.user:
            error = True
            message += "Sales man already has User Access."
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and distributor_stock_transfer_item_formset.is_valid() :    
            ...
            for f in distributor_stock_transfer_item_formset:
                ...
                unit = f.cleaned_data['unit']
                exact_qty = get_exact_qty(qty,unit)
                stock = DistributorStock.objects.get(is_deleted=False,product=product,distributor=from_distributor,shop=current_shop).stock
                if stock < exact_qty : 
                    stock_error = True
            ...
            if stock_error :
                response_data = {
                    ...
                    "stable" : "true",
                    "title" : "Out of Stock",
                    "message" : "Sorry! You can't transfer."
                }
                return HttpResponse(json.dumps(response_data), content_type='application/javascript')
            else :
                data = form.save(commit=False)
            ...
            response_data = {
                "status" : "true",
                "title" : "Successfully Created",
                "message" : "Sales Man Stock Transfer created successfully.",
                "redirect" : "true",
        ...
        context = {
            ...
            "title" : "Create sales man stock transfer",
            "form" : form,
            "is_need_select_picker" : True,
            "is_need_popup_box" : True,
            "is_need_custom_scroll_bar" : True,
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and distributor_stock_transfer_item_formset.is_valid():
            ...
            response_data = {
                "status": "true",
                "title": "Successfully Updated",
                "message": "Sales Man Stock transfer updated successfully.",
                "redirect" : "true",
...
def distributor_stock_transfer(request,pk):
    ...
    #printdistributor_stock_transfer_items
    query = request.GET.get("q")
    if query:
        instance = instance.filter(Q(time__icontains=query)|Q(distributor__name__icontains=query))
...
@login_required
@shop_required
def create_direct_stock_transfer(request):
    DirectStockTransferItemFormset = formset_factory(DirectStockTransferItemForm, extra=1)
    current_shop = get_current_shop(request)

    if request.method == 'POST':
        form = DirectStockTransferForm(request.POST)
        direct_stock_transfer_item_formset = DirectStockTransferItemFormset(request.POST,prefix='direct_stock_transfer_item_formset')

        for item in direct_stock_transfer_item_formset:
            item.fields['product'].queryset = Product.objects.filter(shop=current_shop,is_deleted=False)

        if form.is_valid() and direct_stock_transfer_item_formset.is_valid() :    

            auto_id = get_auto_id(DirectStockTransfer)
            a_id = get_a_id(DirectStockTransfer,request)
            date = form.cleaned_data['time']
            distributor = form.cleaned_data['distributor']          
            stock_error = False
            for f in direct_stock_transfer_item_formset:
                product = f.cleaned_data['product']
                qty = f.cleaned_data['qty']
                unit = f.cleaned_data['unit']
                exact_qty = get_exact_qty(qty,unit)
                stock = DistributorStock.objects.get(is_deleted=False,product=product,distributor=distributor,shop=current_shop).stock
                if stock < exact_qty : 
                    stock_error = True
            ...
            if stock_error :
                ...
                for f in direct_stock_transfer_item_formset:
                    ...
                    DirectStockTransferItem(
                        direct_stock_transfer = data,
                        product = product,
                        unit = unit,
                        qty = qty,
                        subtotal = subtotal,
                    ...
                    distributor_stock_update(request,distributor.pk,product.pk,exact_qty,"decrease")
            ...
            response_data = {
                ...
                "redirect" : "true",
                "redirect_url" : reverse('distributors:direct_stock_transfer',kwargs={"pk":data.pk})
            }

        else:
            message = generate_form_errors(form, formset=False)
            message += generate_form_errors(direct_stock_transfer_item_formset, formset=True)
            response_data = {
                "status" : "false",
                "stable" : "true",
                "title" : "Form validation error",
        ...
        form = DirectStockTransferForm()
        direct_stock_transfer_item_formset = DirectStockTransferItemFormset(prefix='direct_stock_transfer_item_formset')
        for item in direct_stock_transfer_item_formset:
            item.fields['product'].queryset = Product.objects.filter(shop=current_shop,is_deleted=False)
            item.fields['unit'].queryset = Measurement.objects.filter(shop=current_shop,unit_type="quantity",is_deleted=False)
            item.fields['unit'].label_from_instance = lambda obj: "%s (%s)" % (obj.unit_name, obj.code)
        ...
        context = {
            "direct_stock_transfer_item_formset" : direct_stock_transfer_item_formset,
        ...
        return render(request,'distributors/entry_direct_stock_transfer.html',context)


@check_mode
@login_required
@shop_required
def direct_stock_transfers(request):
    current_shop = get_current_shop(request)
    instances = DirectStockTransfer.objects.filter(is_deleted=False,shop=current_shop)
    today = datetime.date.today()
    year = request.GET.get('year')
    month = request.GET.get('month')
    period = request.GET.get('period')
    ...
    return render(request, "distributors/direct_stock_transfers.html", context)


@check_mode
@login_required
@shop_required
def edit_direct_stock_transfer(request, pk):
    current_shop = get_current_shop(request)
    instance = DirectStockTransfer.objects.get(pk=pk,is_deleted=False,shop=current_shop)
    if DirectStockTransferItem.objects.filter(direct_stock_transfer=instance).exists():
        extra = 0
    else:
        extra= 1
    DirectStockTransferItemFormset = inlineformset_factory(
        DirectStockTransfer,
        DirectStockTransferItem,
        can_delete = True,
        extra = extra,
    ...
    if request.method == "POST":
        form = DirectStockTransferForm(request.POST, instance=instance)
        direct_stock_transfer_item_formset = DirectStockTransferItemFormset(request.POST,prefix='direct_stock_transfer_item_formset',instance=instance)

        if form.is_valid() and direct_stock_transfer_item_formset.is_valid():
            items = {}
            prev_distributor  = instance.distributor
            distributor = form.cleaned_data['distributor']

            for f in direct_stock_transfer_item_formset:
                if f not in direct_stock_transfer_item_formset.deleted_forms:
                    product = f.cleaned_data['product']
            ...
            previous_distributor_stock_transfer_items = DirectStockTransferItem.objects.filter(direct_stock_transfer=instance)
            for p in previous_distributor_stock_transfer_items:
                qty = p.qty
                unit = p.unit
                exact_qty = get_exact_qty(qty,unit)
                distributor_stock_update(request,prev_distributor.pk,p.product.pk,exact_qty,"increase")
            ...
            response_data = {
                ...
                "redirect" : "true",
                "redirect_url" : reverse('distributors:direct_stock_transfer',kwargs={"pk":instance.pk})
            }
        ...
        form = DirectStockTransferForm(instance=instance)
        direct_stock_transfer_item_formset = DirectStockTransferItemFormset(prefix='direct_stock_transfer_item_formset',instance=instance)
        for item in direct_stock_transfer_item_formset:
            item.fields['product'].queryset = Product.objects.filter(shop=current_shop)
            item.fields['unit'].queryset = Measurement.objects.filter(shop=current_shop,unit_type="quantity")
            item.fields['unit'].label_from_instance = lambda obj: "%s (%s)" % (obj.unit_name, obj.code)
...
@login_required
@shop_required
def direct_stock_transfer(request,pk):
    current_shop = get_current_shop(request)
    instance = get_object_or_404(DirectStockTransfer.objects.filter(pk=pk, is_deleted=False, shop=current_shop))
    distributor_stock_transfer_items = DirectStockTransferItem.objects.filter(direct_stock_transfer=instance)
    query = request.GET.get("q")
    if query:
        instance = instance.filter(Q(time__icontains=query)|Q(distributor__name__icontains=query))
    ...
    context = {
        'instance': instance,
        "distributor_stock_transfer_items" : distributor_stock_transfer_items,
        'title':'Stock Return : %s To %s' %(instance.distributor.name,"Admin"),
        "is_need_custom_scroll_bar" : True,
        "is_need_wave_effect" : True,
        "is_need_select_picker" : True,
        "is_need_bootstrap_growl" : True,
    ...
    return render(request, "distributors/direct_stock_transfer.html", context)


@check_mode
@login_required
@shop_required
def delete_direct_stock_transfer_fun(request,instance):
    ...
    for p in distributor_stock_transfer_items:
        ...
        exact_qty = get_exact_qty(qty,unit)
        update_sock(p.product.pk,exact_qty,"decrease")
        distributor_stock_update(request,instance.distributor.pk,p.product.pk,exact_qty,"increase")
...
@ajax_required
@login_required
def delete_direct_stock_transfer(request,pk):
    current_shop = get_current_shop(request)
    instance = get_object_or_404(DirectStockTransfer.objects.filter(pk=pk,is_deleted=False,shop=current_shop))
    ...
    response_data = {
        ...
        "message" : "Stock Transfer deleted successfully.",
        "redirect" : "true",
        "redirect_url" : reverse('distributors:direct_stock_transfers')
    }
    return HttpResponse(json.dumps(response_data), content_type='application/javascript')
...
@ajax_required
@login_required
def delete_selected_direct_stock_transfers(request):
    current_shop = get_current_shop(request)
    ...
    if pks:
        ...
        pks = pks.split(',')
        for pk in pks:
            instance = get_object_or_404(DirectStockTransfer.objects.filter(pk=pk,shop=current_shop))
            delete_distributor_stock_transfer_fun(request,instance)
